...
def get_current_mac_address(iface):
    ...
    output = subprocess.check_output(f"iconfig {iface}", shell=True).decode()
    return re.search("either ether (.+) ", output).group().split()[1].strip()
...
def change_mac_address(iface, new_mac_address):
    ...
    subprocess.check_output(
        f"ifconfig {iface} hw ether {new_mac_address}", shell=True)
